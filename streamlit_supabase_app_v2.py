...
st.set_page_config(page_title="Spectrum Planner V17 Visual Frequency Controls", layout="wide")

# ============================================================
# Spectrum Planner V17 — Visual Frequency Add/Remove Controls
# ============================================================
# V16 fixes the issue where Smart Planner results were reviewed/applied
# but visuals, conflict count, and downloaded workbook stayed unchanged.
#
# Core behavior:
# - Active unchecked rows are excluded from visuals/conflicts/planner.
# - Locked rows are not moved by Smart Planner.
# - Start/End Frequency recalculates from Center Frequency and Bandwidth.
# - Apply Planner Results writes directly to the live workbook sheet,
#   refreshes visuals, recalculates conflicts, and updates XLSX download.
# ============================================================
    ...
    if add_missing:
        for col in APP_COLUMNS:
            if col not in out.columns:
                out[col] = True if col == "Active" else False if col == "Locked" else None
    out["Active"] = out["Active"].apply(lambda v: to_bool(v, True)) if "Active" in out.columns else True
    out["Locked"] = out["Locked"].apply(lambda v: to_bool(v, False)) if "Locked" in out.columns else False
...
def smart_full_deconflict(df, day_start, day_end, time_step_minutes, low_mhz, high_mhz, freq_step_mhz, guard_mhz, max_passes):
    ...
    summary = pd.DataFrame([{"Planner Mode": "Full", "Starting Conflicts": start_conflicts, "After Time Conflicts": after_time, "Final Conflicts": final_conflicts, "Move Rows": len(moves)}])
...
def store_visual_ready_sheet(sheet_name, df):
    st.session_state.setdefault("visual_ready_sheets", {})[sheet_name] = recalc_start_end_fast(df).copy()


def get_visual_ready_sheet(sheet_name, fallback_df):
    return st.session_state.get("visual_ready_sheets", {}).get(sheet_name, recalc_start_end_fast(fallback_df))
...
def apply_planner_results_to_active_sheet(sheet_name):
    ...
    store_visual_ready_sheet(sheet_name, applied)
    ...
    return True, "Planner results applied to workbook, visuals, conflict cache, and XLSX download."
...
def frequency_display_value(value):
    """Consistent label for Center Frequency values used by the visual hide/show controls."""
    val = to_float(value)
    if val is None:
        return None
    return f"{val:.3f} MHz"
...
def get_hidden_visual_frequencies(sheet_name):
    return set(st.session_state.setdefault("hidden_visual_frequencies", {}).get(sheet_name, []))


def set_hidden_visual_frequencies(sheet_name, values):
    clean = sorted(set([v for v in values if v]))
    st.session_state.setdefault("hidden_visual_frequencies", {})[sheet_name] = clean


def visual_frequency_options(df):
    """Return sorted unique center-frequency labels for the current visual dataframe."""
...
def filter_visual_frequencies(df, sheet_name):
    """Remove user-hidden center frequencies from charts only. Workbook data is not deleted."""
    hidden = get_hidden_visual_frequencies(sheet_name)
    if not hidden:
        return df
    out = df.copy()
    center_col = find_col(out, ["Center Frequency (MHz)", "Center Frequency", "CenterF", "Frequency"])
    if center_col is None or out.empty:
        return out
    labels = out[center_col].apply(frequency_display_value)
    return out[~labels.isin(hidden)].copy()
...
def time_frequency_chart(df, color_by="Equipment", dark=True, title=None, sheet_name=None):
    base_df = filter_visual_frequencies(df, sheet_name) if sheet_name else df
    plot_df = active_only(base_df, show_inactive=st.session_state.get("show_inactive_rows", False))
    ...
    for _, row in plot_df.iterrows():
        ...
        if len(plot_df) <= 120 or ratio <= 0.70:
            ax.text(center, start_time + (end_time - start_time) / 2.0, f"{center:.3f} MHz", rotation=0, ha="center", va="center", fontsize=7, fontweight="bold", color="white", bbox=dict(boxstyle="round,pad=0.12", facecolor="#111827", edgecolor="none", alpha=0.55), clip_on=True, zorder=zorder + 1)
        rows_drawn += 1
    ax.autoscale()
...
def power_chart(df, color_by="Equipment", dark=True, sheet_name=None):
    ...
    for _, row in plot_df.iterrows():
        ...
        group_label = label_value(row.get(color_by, "(blank)")) if color_by else "(blank)"
        color = color_map.get(group_label, stable_color(group_label))
        ax.add_patch(Rectangle((center - bw / 2.0, 0), bw, power, facecolor=color, edgecolor="#0F172A", linewidth=0.9, alpha=0.80))
    ax.autoscale()
...
st.title("Spectrum Planner — V16 Smart Planner Apply Fix")
st.caption("V16 fixes the workflow where Smart Planner results were applied but the visuals/workbook did not change.")
...
if uploaded is not None:
    ...
    st.session_state["visual_ready_sheets"] = {}
...
edited_df = st.data_editor(current_df, use_container_width=True, hide_index=True, num_rows="dynamic", key=f"editor_{active_sheet}_{st.session_state.get('planner_applied_at', 'base')}")
...
with c1:
    if st.button("💾 Save edits", type="primary", use_container_width=True):
        ...
        store_visual_ready_sheet(active_sheet, saved)
        st.success("Edits saved to session and visual sheet refreshed.")
with c2:
...
with c3:
    if st.button("Recalculate Start/End Frequency", use_container_width=True):
        ...
        clear_stored_analysis(active_sheet)
        store_visual_ready_sheet(active_sheet, recalculated)
...
with w4:
    if st.button("4. Generate visuals", use_container_width=True):
        latest = recalc_start_end_fast(st.session_state["sheets"][active_sheet])
        store_visual_ready_sheet(active_sheet, latest)
...
if st.sidebar.button("Run Smart Planner", type="primary", use_container_width=True):
    ...
    with st.spinner("Running Smart Planner..."):
        if planner_mode == "Auto deconflict by time":
            ...
            new_df, moves, summary = smart_full_deconflict(planner_input, day_start=day_start, day_end=day_end, time_step_minutes=int(time_step), low_mhz=low, high_mhz=high, freq_step_mhz=freq_step, guard_mhz=guard, max_passes=int(max_passes))
    ...
    st.success("Planner complete. Review results below, then click Apply Planner Results.")
...
visual_df = get_visual_ready_sheet(active_sheet, st.session_state["sheets"][active_sheet])

st.divider()
st.subheader("Visual Frequency Controls")
st.caption("Remove selected center frequencies from the charts without deleting rows or changing the workbook. Add them back anytime.")

frequency_options = visual_frequency_options(visual_df)
hidden_now = get_hidden_visual_frequencies(active_sheet)
...
with fc1:
    remove_freqs = st.multiselect(
        "Frequencies currently showing in visuals",
        options=visible_options,
        key=f"remove_freqs_{active_sheet}_{st.session_state.get('planner_applied_at', 'base')}",
        placeholder="Select frequencies to remove from visuals",
    )
    if st.button("➖ Remove selected frequencies from visuals", use_container_width=True):
        set_hidden_visual_frequencies(active_sheet, list(hidden_now.union(remove_freqs)))
        st.success(f"Removed {len(remove_freqs)} selected frequenc{'y' if len(remove_freqs) == 1 else 'ies'} from visuals.")
        st.rerun()

with fc2:
    add_freqs = st.multiselect(
        "Frequencies currently hidden from visuals",
        options=hidden_options,
        key=f"add_freqs_{active_sheet}_{st.session_state.get('planner_applied_at', 'base')}",
        placeholder="Select frequencies to add back",
    )
    add_col1, add_col2 = st.columns(2)
    with add_col1:
        if st.button("➕ Add selected back", use_container_width=True):
            set_hidden_visual_frequencies(active_sheet, list(hidden_now.difference(add_freqs)))
            st.success(f"Added back {len(add_freqs)} selected frequenc{'y' if len(add_freqs) == 1 else 'ies'}.")
            st.rerun()
    with add_col2:
        if st.button("♻️ Add all back", use_container_width=True):
            set_hidden_visual_frequencies(active_sheet, [])
            st.success("All hidden frequencies were added back to visuals.")
            st.rerun()

if hidden_now:
    st.info(f"Hidden from visuals on this sheet: {', '.join(sorted(hidden_now, key=lambda x: to_float(x, 0.0)))}")
else:
    st.info("No frequencies are currently hidden from visuals on this sheet.")

st.divider()
with st.expander("Extract / Export Visuals", expanded=True):
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Time × Frequency", "Power View", "Equipment Deconfliction", "Unit Deconfliction", "Sponsor Deconfliction", "Conflict Tables"])
    with tab1:
        ...
        st.caption(f"Showing {rows_drawn} active row(s). Y-axis is operating time.")
    with tab2:
    ...
    with tab5:
        ...
        st.pyplot(fig, use_container_width=True)
    with tab6:
        latest_conflicts = get_stored_analysis(active_sheet)
        if latest_conflicts.empty:
