...
# - V49: Clean layout mode removes the cluttered command-dashboard mockup and keeps one simple workspace with tabs.
# - V50: 3-click UX redesign with five primary work areas, quick actions, and daily-task dashboard.
# - V51: Adds Locked column after Active; locked rows are included but frequency/time/bandwidth are protected from allocator changes.
# - V52: Adds visible Lock Frequency / Lock Time / Lock Both controls and protects locked fields from Smart Planner changes.
# - V53: Manual frequency edits auto-recalculate Start/End Frequency from Center Frequency and Bandwidth while respecting locks.
...
with main_tab_allocation:
    v50_section("Allocation Manager", "Turn systems on/off, lock equipment, edit rows, and search. Lock Frequency protects the frequency fields; Lock Time protects the time fields. Manual Center Frequency or Bandwidth edits automatically update Start and End Frequency.")
    
    st.markdown("**Frequency tools**")
    if st.button("Recalculate Start/End from Center + Bandwidth", use_container_width=True, key="v53_recalc_freq_ranges"):
        try:
            recalced_df = recalc_all_unlocked_frequency_ranges(current_df_all if "current_df_all" in locals() else current_df)
...
with tab2:
    st.markdown("""<div class="pcc-panel"><div class="pcc-panel-title">Time × Frequency — By Equipm
ent</div><div class="pcc-panel-caption">Boxes display frequency only; equipment names remain in the legend.</div></div>""", unsafe_allow_html=True)
